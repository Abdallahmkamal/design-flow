...
19. `docs/ui/team-ready-ui-handoff.md` for the approved post-MVP modernization workstream

If code and documentation disagree, stop and surface the mismatch. Do not silently choose one.

Product behavior is governed by the product specifications and approved decisions. Vodafone color and typography are governed by `docs/design-system.md`, with its linked foundation sources remaining authoritative for those extracted facts. D-077 through D-099 and verified Astryx references remain the completed MVP/legacy presentation baseline. D-109 and `docs/ui/team-ready-ui-handoff.md` govern new or migrated team-ready surfaces, using Design Flow-owned shadcn source and Tailwind without changing Vodafone color/typography authority. Design Flow's UI architecture and component-ownership rules are governed by `docs/ui-architecture.md`; stable cross-product UI direction and feature-brief expectations are governed by `docs/ui-direction.md`.
...
For the team-ready workstream, the preceding Astryx presentation workflow applies only to unmigrated legacy components. New/migrated surfaces follow D-109/D-110 and the approved handoff. shadcn is source-owned starting code rather than visual authority; Tailwind and CSS Modules may coexist, and global Tailwind Preflight remains disabled while legacy screens coexist.
...
- Reuse Vodafone semantic color tokens and text styles. Legacy components use the approved Astryx mappings; team-ready components map the same Vodafone authority through the D-109 Tailwind/shadcn layer and approved handoff.
- For legacy components, continue using CSS custom properties and CSS Modules as documented. For team-ready migrated surfaces, Tailwind utilities and Design Flow-owned shadcn source are permitted; CSS Modules may coexist and global Tailwind Preflight remains disabled until legacy migration is complete. Neither path may introduce a parallel color/typography authority or undocumented visual values.
...
- Give every shared UI component a documented public API, Vodafone color/typography, presentation traceable to its applicable legacy or team-ready authority, accessible interaction behavior, responsive behavior where relevant, and automated tests.
