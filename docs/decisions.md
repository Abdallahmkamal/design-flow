...
**Last amended:** 2026-08-11 — D-111 refines the team-ready mobile shell action presentation
...
| D-111 | Team-ready mobile shell glass dock | Refine only the compact shell presentation: keep the existing role-filtered destinations, routes, global-action permissions, safe-area behavior, and Sheet/Dialog accessibility mechanics, but replace the opaque bottom footer and bottom-sheet visual with a transparent fixed region containing a frosted-glass navigation capsule and separate glass/white FAB. The resting FAB uses a restrained CSS-only red perimeter glow; opening transforms it into a calm close state and raises Log Work then Create Ticket as a staggered floating action group over a light neutral backdrop. The dock remains visible, page content scrolls beneath it, final content remains reachable above it, Safari blur has a fallback, and reduced-motion disables continuous/rotational motion. This refines the mobile-shell presentation portion of D-110 and `docs/ui/team-ready-ui-handoff.md` without changing product behavior, authorization, or desktop navigation. |
