...
| D-094 | Bootstrap and rollout | Bootstrap the first account as Manager + Admin so the initial active hierarchy is valid, then establish organizational/reference data through an auditable procedure; use no automatic historical import, then run staging acceptance, a limited production pilot, full-team launch, and two-week stabilization behind explicit security, recovery, permission, workflow, reporting, and runbook gates; Admin owns system operation while Manager owns the organizational view. D-104 supersedes only the staging-acceptance and pilot durations. |
