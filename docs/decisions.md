...
**Last amended:** 2026-07-19 — D-097 records the independent Create New Ticket and optional status paths launched from Log Work
...
| D-074 | Visual foundation | `docs/design-system.md` is the visual source of truth for Vodafone color, semantic tokens, typography, spacing, elevation, and brand identity; it does not prescribe the runtime component implementation |
| D-075 | Status appearance | Centralized Design Flow semantic aliases map the six statuses plus Blocked/Archived indicators in Light/Dark modes; badges use background and text only, with no border, and may be remapped without changing persisted status data |
| D-076 | Design-system architecture | Vodafone Foundations provide the visual layer, distilled Astryx notes provide advisory engineering knowledge, and Design Flow owns the component implementation; these layers are complementary, not competing systems |
...
| D-078 | Design Flow UI library | Shared components are implemented and owned under `src/ui/`, use Design Flow/Vodafone tokens, expose documented project APIs, and require accessibility, behavior, responsive, and automated-test coverage |
...
| D-080 | Foundation gaps | When Vodafone has no current value for radius, motion, sizing, or another foundation, Design Flow may define a centralized semantic value informed by Astryx guidance and record it in `docs/design-system.md` before use |
...
| D-087 | Styling delivery | Represent approved Vodafone/Design Flow tokens as CSS custom properties and scope component styles with CSS Modules; these mechanisms do not create a second design system, and components may not introduce undocumented token values; use neither Tailwind nor CSS-in-JS in the MVP |
...
| Vodafone and Astryx as competing design systems | Vodafone visual foundations plus Astryx behavior guidance plus Design Flow implementation ownership |
