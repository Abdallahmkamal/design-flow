...
**Last amended:** 2026-07-19 — D-099 assigns Vodafone color/typography and Astryx non-color presentation authority
...
| D-096 | Phase 1 foundation gaps | Refined by D-099: the Phase 1 element/container/overlay/full radii at 4/8/12/999px, control heights at 32/40/48/64px, 2px focus ring with 2px offset, and 700ms spinner duration are provisional and require Astryx fidelity review before Phase 2 UI implementation. `Vodafone VF` remains the required family, with the system fallback local-only until an approved licensed font asset is supplied |
