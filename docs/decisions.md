...
**Last amended:** 2026-07-29 — D-105 accepts two explicit staging exceptions
...
| D-105 | Phase 7 staging acceptance exceptions | Accept the two owner-created inactive test profiles as outside the active nine-reserved-persona acceptance set; they remain inactive and must not be used as acceptance principals. Accept the observed 390 px Reports page-level horizontal overflow as nonblocking for the two-day staging gate because the report remains available and desktop behavior passes. Keep the overflow visible as a known usability defect that must be corrected and retested before full-team release. These explicit exceptions count as approved outcomes in the otherwise completed Day 1 matrix; they do not waive authorization, security, recovery, permission, data-integrity, or production gates. |
| D-106 | Monitored production rollout without a pilot gate | Supersede D-094/D-104 only where they require a fixed five-person limited-production roster and two passing pilot days before release. After staging acceptance, bootstrap/recovery, current quota review, launch-blocker correction/retest, and explicit release authorization pass, onboard real team members as they are ready. Treat the first two working days after release as monitored operation inside the two-week stabilization period, not as a release or Phase 7 exit gate; never relabel the unperformed former pilot as passed. Review authentication, authorization, notification isolation, reports/source reconciliation, provider logs/quotas, and daily encrypted backups during those days. A security, authentication, authorization, recovery, data-integrity, or core-workflow blocker triggers the incident/pause procedure; ordinary UX findings enter stabilization. Preserve Admin/Manager separation, the D-105 Reports correction before full-team release, explicit external-action authorization, and all security/recovery controls. |
