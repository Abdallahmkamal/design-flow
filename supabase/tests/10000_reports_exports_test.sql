...
select throws_ok($$select public.export_report_rows('ticket_summary','{}')$$,'P0001','DF_FORBIDDEN','Viewer cannot export CSV');
...
select is((public.get_reports(jsonb_build_object('tab','designers','periodStart',current_date-10,'periodEnd',current_date))->>'canExport')::boolean,false,'Designer has no CSV capability');
...
select throws_ok($$select public.export_report_rows('ticket_activity','{}')$$,'P0001','DF_FORBIDDEN','Designer cannot bypass CSV denial');
reset role;

select set_config('request.jwt.claim.sub','10000000-0000-4000-8000-000000000003',true);
set local role authenticated;
select is(public.get_reports()->>'defaultScopeKey','me','Designer Admin retains Me default');
select is((public.get_reports()->>'canExport')::boolean,true,'Designer Admin receives CSV capability');
select is(jsonb_array_length(public.export_report_rows('ticket_activity',jsonb_build_object('scopeKey','all','periodStart',current_date-10,'periodEnd',current_date))->'rows'),3,'Ticket activity CSV includes every matching entry');
...
select ok(public.export_report_rows('designer_summary',jsonb_build_object('periodStart',current_date-10,'periodEnd',current_date))->'rows'->0 ? 'workEmail','Authorized designer summary includes work email projection');
select is(jsonb_array_length(public.export_report_rows('ticket_activity',jsonb_build_object('scopeKey','all','areaIds',jsonb_build_array('50000000-0000-4000-8000-000000000002'),'periodStart',current_date-10,'periodEnd',current_date))->'rows'),0,'Ticket activity CSV applies the current Area filter');
...
select lives_ok($$select public.export_report_rows('ticket_summary','{}')$$,'Lead exports CSV');
...
set local role authenticated;
select lives_ok($$select public.export_report_rows('designer_ticket','{}')$$,'Lead Admin exports designer-ticket CSV');
reset role;

select set_config('request.jwt.claim.sub','10000000-0000-4000-8000-000000000006',true);
set local role authenticated;
select is(public.get_reports()->>'defaultScopeKey','manager:10000000-0000-4000-8000-000000000006','Manager keeps group default');
select lives_ok($$select public.export_report_rows('ticket_activity','{}')$$,'Manager exports ticket activity CSV');
reset role;
...
select throws_ok($$select public.export_report_rows('ticket_summary','{}')$$,'P0001','DF_PASSWORD_CHANGE_REQUIRED','Password-restricted principal cannot export');
reset role;
